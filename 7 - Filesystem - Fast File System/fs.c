...
uint BBLOCK(uint current)
{
  ...
  return bm_start + block_in_bg / BG_SZ;
}
...
static uint
balloc(uint dev, int pref_bg)
{
  struct buf *bp, *info;
  int m, i, j, bit;
  uint current_bg_start, bm_block;

  for(i = 0; i < BG_COUNT; i++){

    current_bg_start = BG_START + ((pref_bg + i) % BG_COUNT) * BG_SZ;
    for(j = 0; j < DATA_BM_BLOCKS; j++){

      bm_block = (current_bg_start + METABLOCKS - DATA_BM_BLOCKS + j);
      bp = bread(dev, bm_block);
      for(bit = 0; bit < DATA_BLOCKS_PER_BG; bit++){
        m = 1 << (bit % 8);
        if((bp->data[bit / 8] & m) == 0){
          bp->data[bit / 8] |= m;            // Mark block in use.

          info = bread(dev, GET_INFO_BLOCK(bm_block));
          ((bg_t*)info->data)->data_blocks_in_use++;
          brelse(info);

          log_write(bp);
          brelse(bp);
          bzero(dev, current_bg_start + METABLOCKS + bit);
          return current_bg_start + METABLOCKS + bit;
        }
      }
...
bfree(int dev, uint b)
{
  ...
  bi = ((b - BG_START) % BG_SZ) - METABLOCKS;
  m = 1 << (bi % 8);
    
  if((bp->data[bi / 8] & m) == 0)
      panic("freeing free block");

  info = bread(dev, GET_INFO_BLOCK(b));
  ((bg_t*)info->data)->data_blocks_in_use--;
  brelse(info);
...
ialloc(uint dev, short type)
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++)
  {
    ...
    if(dip->type == 0){  // a free inode
      info = bread(dev, GET_INFO_BLOCK(IBLOCK(inum)));
      ((bg_t*)info->data)->inodes_in_use++;
      brelse(info);
...
iupdate(struct inode *ip)
{
  ...
  dip->nlink = ip->nlink;
  if(ip->nlink <= 0){
    info = bread(ip->dev, GET_INFO_BLOCK(IBLOCK(ip->inum)));
    ((bg_t*)info->data)->inodes_in_use--;
    brelse(info);
  }
  dip->size = ip->size;
...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    if((addr = ip->addrs[bn]) == 0){
      addr = balloc(ip->dev, BG_IDX(IBLOCK(ip->inum)));
      ip->addrs[bn] = addr;
    }
    return addr;
  }
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = ip->addrs[NDIRECT]) == 0)
      ip->addrs[NDIRECT] = addr = balloc(ip->dev, BG_IDX(IBLOCK(ip->inum)));
      
    bp = bread(ip->dev, addr);
    a = (uint *)bp->data;
      
    if((addr = a[bn]) == 0){
      a[bn] = addr = balloc(ip->dev, BG_IDX(IBLOCK(ip->inum)));
      log_write(bp);
    }
    brelse(bp);
    return addr;
