...
printf("DEBUG %d", (NINDIRECT*NINDIRECT));
...
iappend(uint inum, void *xp, int n)
{
  ...
  uint indirect2[NINDIRECT];//for doubly indirect
  uint x;

  rinode(inum, &din);

  off = xint(din.size);
  while(n > 0){ //while there are bytes to write
    fbn = off / 512; //get current block number
    assert(fbn < MAXFILE);
	//direct
    if(fbn < NDIRECT){
